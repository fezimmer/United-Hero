...
<?php
	if($imgUploaded){
		...
		$fkCategoryID 	= $_REQUEST['fkCategoryID'];

		$lastSortNum = q1("SELECT MAX(fldSortOrder) FROM tblImage WHERE fkCategoryID = $fkCategoryID");
		$fldSortOrder = $lastSortNum + 1;
		if ($fldTitle){
			$success = qr("INSERT INTO tblImage (fldTitle, fldFilename, fldSortOrder, fkCategoryID, fkCreatorID, fldCreateDate, fkEditorID, fldUpdateDate) VALUES (\"$fldTitle\", \"$newfilename\", $fldSortOrder, $fkCategoryID, $loggedInUserID, NOW(), $loggedInUserID, NOW())");
		}

		echo '<img src="/i/success.gif" width="16" height="16" border="0" style="marin-bottom: -4px;" /> Success! You can continue uploading new images above. <br /><img src="'.$upload_image.'" border="0" />';
	}else{
		echo '<img src="/i/error.gif" width="16" height="16px" border="0" style="marin-bottom: -3px;" /> Error(s) Found: ';
