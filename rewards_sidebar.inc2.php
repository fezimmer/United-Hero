...
function jsUpload(upload_field, activeIndex)
{
    ...
    var re_text = /\.jpg|\.gif|\.png|\.bmp/i;
    var filename = upload_field.value;

    /* Checking file type */
    if (filename.search(re_text) == -1)
    {
        alert("File is not a valid extension");
...
<input type="hidden" name="fileframe" id="fileframe" value="true">
<input type='hidden' name='activeIndex' id='activeIndex' value='' />
            ...
            <div id="linkList">
                <?
                    ...
                    for($i;$i<$finalCount;$i++){
                        ...
                        echo "<label for='rewardTitle" . $i . "'>Title*</label><br/>" .
                            "    <input value='" .$rewardsTitle[$i] . "' type='text'";
                        echo $readOnly;
                            echo "name='rewardTitle" . $i . "' id='rewardTitle" . $i . "' size='23' class='required' minlength='2'/><br/><br/>" .
                            "    <label for='rewardDesc" . $i . "'>Description*</label><br/><textarea name='rewardDesc" . $i . "'";
                            echo $readOnly;
                            echo "id='rewardDesc" . $i . "' cols='23' rows='5' maxlength='200' class='required' onKeyDown='limitText(this.form.rewardDesc" . $i . ",this.form.countdown" . $i . ",200);' onKeyUp='limitText(this.form.rewardDesc" . $i . ",this.form.countdown" . $i . ",200);'>";
                        if($rewardsDesc[$i] != '' && $rewardsDesc[$i] != null)echo $rewardsDesc[$i];
                        echo "</textarea><br/><br/>" . "<input type='hidden' id='countdown" . $i . "' value='200'>" .
                            "    <div style='width:48%; float:left;margin-top:4px;'>" .
                            "    <label for='rewardSupport" . $i . "'>Purchase Price*</label><br/>" .
                            "        $<input value='" .$rewardsSupport[$i] . "' type='text'";
                            echo $readOnly;
                            echo "name='rewardSupport" . $i . "' id='rewardSupport" . $i . "' size='5' class='required'/><br/>" .
                            "    </div>" .
						...
						echo "id='rewardMonth" . $i . "'>" .
							"<label for='rewardMonth" . $i . "'><option class='required' value='".$rewardsMonth[$i]."' SELECTED>";
						if($rewardsMonth[$i] != "" && $rewardsMonth[$i] != null) echo getRewardMonth($rewardsMonth[$i]);
						else echo "Month*<br/>";
						echo "</option></label><option value='01'>JANUARY (01)</option>" .
						...
						echo "id='rewardYear" . $i . "'>" .
							"<label for='rewardYear" . $i . "'><option class='required' value='".$rewardsYear[$i]."' SELECTED>";
						if($rewardsYear[$i] != "" && $rewardsYear[$i] != null) echo $rewardsYear[$i];
						else echo "Year*";
						echo "</option></label><option value='2012'>2012</option>" .
							...
							"    </span>" .
							"    <br/><br/>";
						if ($fileName != "") $valInBox = $fileName; else $valInBox = "none";
						if (!$readOnlyKeyword){
							echo "    Image Upload";
							echo "<input type=\"file\" name=\"rewardImageFile{$i}\" id=\"rewardImageFile{$i}\" onChange=\"jsUpload(this, $i)\">\n";
							echo "<span id=\"imagePlaceholder{$i}\"></span>\n";
							echo "<input type=\"hidden\" name=\"rewardImage{$i}\" id=\"rewardImage{$i}\" value=\"$valInBox\" disabled>\n";
